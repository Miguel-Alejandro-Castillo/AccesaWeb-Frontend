export default {
  en: {
    'accept': 'Accept',
    'accessa-web': 'AccesaWeb',
    'activate': 'Activate',
    'active-modules': 'Active Modules',
    'active-settings': 'Active Settings',
    'add-module': 'Add Module',
    'blackWhite': 'White on black',
    'blackYellow': 'Yellow on black',
    'yellowBlack':'Black on yellow',
    'whiteBlack':'black on white',
    'center': 'Center',
    'commands': 'Commands',
    'command': 'Command',
    'continuous-mode': 'Continuous Recognition',
    'continuous-mode-description': 'Select it in noise free environments and when you will only speak commands',
    'context': 'Context',
    'default-settings': 'Default settings',
    'desactivate': 'Desactivate',
    'description': 'Description',
    'down': 'Down',
    'english': 'English',
    'error-title-importing-module': 'Error when trying to import the module',
    'error-description-check-out-docs': 'Check out the docs in order to get help.',
    'error-description-importing-module': 'Seems that the module structure is invalid, please fix the module structure and try import it again.',
    'help': 'Help',
    'hide': 'Hide',
    'intermittent-mode': 'Press Ctrl before say every command',
    'intermittent-mode-description': 'Select it in noisy environments or when you want to have full control over the speech recognition start and stop',
    'justify': 'Justify',
    'left': 'Left',
    'module-docs': 'Module Docs',
    'my-setting':'My Setting',
    'none': 'None',
    'on': 'On',
    'off': 'Off',
    'remove': 'Remove',
    'reset-extension':'Reset Settings',
    'right': 'Right',
    'root': 'Main',
    'select-a-contrast': 'Select a contrast:',
    'select-a-font': 'Select a font:',
    'select-a-orientation': 'Select a orientation:',
    'select-a-spacing': 'Select a spacing:',
    'show': 'Show',
    'spanish': 'Spanish',
    'speech-recognizer-mode': 'Speech Recognition Options',
    'speech-recognizer-mode-description': 'The following options allow you to specify how the voice commands have to been recognized.',
    'test': 'Test',
    'test-description': 'Test the above commands over the following elements',
    'turned-on': 'Turned On',
    'turned-off': 'Turned Off',
    'unselected': 'Unselected',
    'up': 'Up'
  },
  es: {
    'accept': 'Aceptar',
    'accesa-web': 'AccesaWeb',
    'accesa-web-description': 'Herramienta de ayuda para las personas con discapacidad.',
    'activate': 'Activar',
    'active-modules': 'Módulos Activos',
    'active-settings': 'Configuraciones Activas',
    'add-module': 'Agregar Módulo',
    'whiteBlack':'negro sobre blanco',
    'yellowBlack':'negro sobre amarillo',
    'blackWhite': 'Blanco sobre negro',
    'blackYellow': 'amarillo sobre negro',
    'center': 'Centrado',
    'commands': 'Comandos',
    'command': 'Comando',
    'continuous-mode': 'Reconocimiento continuo',
    'continuous-mode-description': 'Seleccionarlo en lugares silenciosos y cuando solo vas a decir comandos de voz',
    'context': 'Contexto',
    'default-settings':'Configuiracion predeterminada',
    'desactivate': 'Desactivar',
    'description': 'Descripción',
    'down': 'Abajo',
    'english': 'Inglés',
    'error-description-importing-module': 'Al parecer la estructura del módulo es inválida, por favor corrígela e intenta importar el módulo nuevamente.',
    'error-description-check-out-docs': 'Revisar la documentación en caso de necesitar ayuda.',
    'error-title-importing-module': 'Error al intentar importar el módulo',
    'help': 'Ayuda',
    'hide': 'Ocultar',
    'intermittent-mode': 'Presionar Ctrl antes de decir cada comando',
    'intermittent-mode-description': 'Seleccionarlo en ambientes ruidosos o cuando quieras tener control de cuando el reconocimiento de voz inicia y termina',
    'justify': 'Justificar',
    'left':'Izquierda',
    'module-docs': 'Documentación de los Módulos',
    'my-setting': 'Mi configuración',
    'none': 'Ninguno',
    'on': 'Encender',
    'off': 'Apagar',
    'remove': 'Eliminar',
    'reset-extension':'Restablecer Configuración',
    'right': 'Derecha',
    'root': 'Principal',
    'select-a-contrast': 'Selecciona un contraste:',
    'select-a-font': 'Selecciona una fuente:',
    'select-a-orientation': 'Selecciona una orientación:',
    'select-a-spacing': 'Selecciona un espaciado:',
    'show': 'Mostrar',
    'spanish': 'Español',
    'speech-recognizer-mode': 'Opciones de Reconocimiento de Voz',
    'speech-recognizer-mode-description': 'Las siguientes opciones habilitan a especificar como los comandos de voz deben ser reconocidos.',
    'test': 'Prueba',
    'test-description': 'Prueba los comandos sobre los siguientes ejemplos',
    'turned-on': 'Encendido',
    'turned-off': 'Apagado',
    'unselected': 'Sin seleccionar',
    'up':'Arriba'
  }
};
