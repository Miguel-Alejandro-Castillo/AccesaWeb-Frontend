...
import { isColorInput } from '../element_selectors';
...
function handleSwitchOnSelectElement(state) {
  return isColorInput(state.selectedElement);
}
...
function reviewCommand(state, command, background, allTheCommands) {
  ...
  if (_.isInteger(number) && number >= 1 && number <= 30) {
    const newState = {
      ...state,
    ...
    if (newState.x && newState.y) {
      ...
      state.selectedElement.value = newState.selectedColor;
    }
    return newState;
  }
}
...
function setup(state) {
  return {
    colors: colors,
    selectedColor: getSelectedColor(state.selectedElement),
    x: 0,
    y: 0,
